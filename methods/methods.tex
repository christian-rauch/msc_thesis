...
%\centering
%%\subfloat[MultiSense SL]{\includegraphics[height=4cm]{images/valkyrie/MultiSense_SL_rotated.jpg} \label{fig:multisense}}
%\subfloat[MultiSense SL]{\includegraphics[height=4cm]{images/valkyrie/MultiSense_SL.jpg} \label{fig:multisense}}
%\subfloat[Asus Xtion PRO]{\includegraphics[width=0.5\textwidth]{images/valkyrie/xtion_pro_live.jpg} \label{fig:xtion_pro}}
%\caption{Depth perception systems (Image credits: (a) Carnegie Robotics, (b) Asus)}
%\label{fig:depth_sensors}
%\end{figure}
...
\section{Robot Model Representation}

Part of this thesis is the adaptation of the reference implementation to robot model representations stored in URDF (Unified Robot Description Format) format. URDF defines at least a tree structure that resembles the kinematic tree of a robot with its joints and links. In addition, it can provide among other things information about joint limits and the geometric properties of the links.
...
\caption{Local model SDF and observation SDF (Inspired by \cite{Schmidt2015} figures 3 and 6). (a) the observed point is assigned to \emph{part 2}, (b) the estimated box (light gray) lies in between the observation and the image centre and thus invalidates the free space constrain.}
\end{figure}
